...
/*
Calculates the cost from start to goal using GREEDY.
Parameters to the function:
npStart : Position - the starting position.
npGoal : Position - the goal position.
autoCompute : bool - compute the GREEDY node expansion until it finishes searching for the goal or goal is not found. If false, we get step by step results of the GREEDY nodes expansion.
...
*/
double /*startToGoalCost*/ SquareGreedy(Position npStart, Position npGoal, bool autoCompute, Position topLeft, Position bottomRight, const int showMap, std::vector<Position> & goalPath)
{
	if (!SquarePreSearch(priorityFrontier, npStart, npGoal, topLeft, bottomRight, 0.00f))
		return -1;
	...
	//If autoCompute is not set to run (meaning we are going to get step by step results of the GREEDY).
	...
	SquareSearchLoop(true, priorityFrontier, PathfindingAlgo::ALGO_GREEDY, npStart, npGoal, topLeft, bottomRight, CostType::COST_TO_GOAL_HEURISTIC, nheuristic, generateInstantaneous, autoCompute, showMap);

	if (!SquarePostSearch(PathfindingAlgo::ALGO_GREEDY, COST_MAX_TERM, npStart, npGoal, goalPath, generateInstantaneous, autoCompute, showMap))
		return -1;
