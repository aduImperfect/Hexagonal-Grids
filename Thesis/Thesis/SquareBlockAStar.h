...
{
	//CURRENTLY RUNNING SAME AS ASTAR. CODING OF THE FUNCTIONALITY REMAINING.
	...
	//If autoCompute is not set to run (meaning we are going to get step by step results of the BLOCKASTAR).
	...
	SquareSearchLoop(true, priorityFrontier, PathfindingAlgo::ALGO_BLOCKASTAR, npStart, npGoal, topLeft, bottomRight, CostType::COST_TOTAL_HEURISTIC, nheuristic, generateInstantaneous, autoCompute, showMap);

	if (!SquarePostSearch(PathfindingAlgo::ALGO_BLOCKASTAR, COST_MAX_TERM, npStart, npGoal, goalPath, generateInstantaneous, autoCompute, showMap))
		return -1;
